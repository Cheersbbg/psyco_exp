...
class NeighborFinder:
    def __init__(self, adj_list, uniform=False, smart=False):
        """
        Params
        ------
        node_idx_l: List[int]
        ...
        self.smart = smart
        self.tgan_instance = None

    def set_tgan_instance(self, tgan_instance):
        self.tgan_instance = tgan_instance
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbors=20):
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ...
            if len(ngh_idx) > 0:
                last_edge_index = ngh_eidx[-1] if ngh_ts[-1] < cut_time else None
                edge_as_time.append(last_edge_index)

                if self.uniform:
                    if self.smart:
                        print("wow")

                        matrix = self.tgan_instance.memoery_passing.memoery_matrix[src_idx]
                        #if matrix doesnt contain all 0 values 
                        print("matrix", matrix.shape) #wow
                        if (matrix == 0).all():
                            print("The matrix is empty (all elements are zero).")
                        else:
                            print("The matrix is not empty (there are non-zero elements).")
                
                            
                        #matrix is torch.float32
                        nodescore = torch.matmul(matrix, torch.ones(matrix.shape[1], 1).to("mps"))
                        if torch.any(nodescore != 0):
                            print("nodescore", nodescore)


                        if matrix.sum().item() != 0:
                            print("memoery_matrix is not zero")
        ...
        return out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch, edge_as_time
