...
def main():
    ...
    def run_exp(exp_name,stim_list, numexp, win, timedif, shuffle=False, emptystim=None, hasbreak=False, numrest=0, break_interval=2,trial_interval=8):
    #keys = event.waitKeys(keyList=['space'])
        ...
        for i in range(numexp):
            ...
            for idx in range(0, len(permu_list), 2):
                print("idx pair:", idx, idx+1)
            # Treat current and next stimulus as a pair
                stim1 = stim_list[permu_list[idx]]
                stim2 = stim_list[permu_list[idx + 1]] if idx + 1 < len(permu_list) else None
                stim1.draw()
                ...
                win.flip()
                core.wait(numsecs/2) #visual presentation seconds
                stim2.draw()
                win.flip()
                core.wait(numsecs/2) #visual presentation seconds
                ...
                shared_queue.put([permu_list[idx], stim_timestamp, trial_multiplier])
                shared_queue.put([permu_list[idx + 1], stim_timestamp, 0])
    ...
    for i, stim_list in enumerate(stim_matrix):
        if testing_with_wifi:
            ...
            time_diff = 0
        run_exp(exp_names[i], stim_list, numexp=numexp, win=win, shuffle=True,hasbreak=True,emptystim=emptystim,timedif=time_diff,break_interval=2,trial_interval=4)
        print(i, "experiment")
        core.wait(5)
...
def runstream(eeg_inlet, num_samples, batch_size=1,exp_name="doublefinger_1sec"):
    global exp_running
    ...
    while exp_running:
        ...
        if not shared_queue.empty():
            ...
            filename = filename_template.format("doubleprocess")
