...
def main():
    ...
    #keys = event.waitKeys(keyList=['space'])
        ...
        permu_list.append(400) #blank stimulus is 200
        permu_list.append(400)
        core.wait(3.5)
        #timer = core.Clock()
        exp_running=True
        for i in range(numexp):
            ...
            for idx in permu_list:
                ...
                win.flip()
                core.wait(1) #random interval seconds
    ...
    for i, stim_list in enumerate(stim_matrix):
        if testing_with_wifi:
            ...
            time_diff = 0
        print(stim_list)
        run_exp(exp_names[i], stim_list, numexp=numexp, win=win, shuffle=True,hasbreak=True,emptystim=emptystim,timedif=time_diff,break_interval=4,trial_interval=2)
        print(i, "experiment")
        core.wait(5)
...
def runstream(eeg_inlet, num_samples, batch_size=1,exp_name="asecondhandsao_meg"):
    global exp_running
